...
def u_activity(request):
    ...
    if request.method == 'GET':
        ...
        if 'del' in request.POST:
            ...
            pid = postdata.get('pay')
            uname = postdata.get('nam')
            uadd = postdata.get('add')
            umob = postdata.get('mob')
            umail = postdata.get('mail')
            uaadhar = postdata.get('aadhar')
            uuser = postdata.get('user')
            uque = postdata.get('que')
            umem = postdata.get('member')
            utrip = postdata.get('trip')
            uprice = postdata.get('price')
            uamount = postdata.get('amount')
...
def user_feedback(request):
    ...
    if request.method == "GET":
        ...
        elif 'upfeed' in request.POST:
            fid = request.POST.get('fe_id')
            fdata = collection.find_one({'f_id': fid})
            data['feed_data'] = fdata
            data['feed'] = fid
            return render(request, 'admin/a_feedup.html', data)
        elif 'update' in request.POST:
            postdata = request.POST
            fid = postdata.get('id')
            ffname = postdata.get('fname')
            mail = postdata.get('fmail')
            que = postdata.get('fque')
            print(fid, ffname, mail, que)
            origin = {'f_id': fid}
            upd = {
                '$set': {'name': ffname, 'mail_id': mail, 'query': que
                 }}
            collection.update_one(origin, upd)
            return redirect('feed')  
...
def user_profile(request):
    ...
    if request.method == "GET":
        ...
        if 'user' in request.POST:
            ...
            postdata = request.POST
            recid = postdata.get('rid')
            name = postdata.get('uname')
            uadd = postdata.get('uadd')
            umob = postdata.get('umob')
            umail = postdata.get('umail')
            uaadhar = postdata.get('uaad')
            uuser = postdata.get('user_rec')
            rpass = postdata.get('upass')
            one = collection.find_one({'username': uuser})
            data['r_data'] = one
...
def user_booking(request):
    ...
    if request.method == "GET":
        ...
        if 'book' in request.POST:
        ...
        elif 'upbook' in request.POST:
            bid = request.POST.get('bo_id')
            fdata = collection.find_one({'pay_id': bid})
            data['book_data'] = fdata
            return render(request, 'admin/a_bookup.html', data)
        elif 'update' in request.POST:
            postdata = request.POST
            bid = postdata.get('bpay')
            ffname = postdata.get('bname')
            add = postdata.get('badd')
            pho = postdata.get('bpho')
            mail = postdata.get('bmail')
            adh = postdata.get('badh')
            user = postdata.get('buser')
            tri = postdata.get('btrio')
            que = postdata.get('bque')
            mem = postdata.get('bmem')
            pri = postdata.get('bpri')
            amo = postdata.get('bamo')
            print(ffname, mail, que)
            origin = {'pay_id': bid}
            upd = {
                '$set': {'name': ffname, 'address': add, 'phone': pho, 'mail_id': mail, 'aadhar_no': adh, 'username': user, 'trip': tri, 'query': que, 'no_of_member': mem, 'trip_price': pri, 'amount': amo
                 }
            }
            collection.update_one(origin, upd)
            return redirect('book')
        elif 'bookrec' in request.POST:
            postdata = request.POST
            bid = postdata.get('bo_id')
            pid = postdata.get('pay')
            uname = postdata.get('nam')
            uadd = postdata.get('add')
            umob = postdata.get('mob')
            ...
            uprice = postdata.get('price')
            uamount = postdata.get('amount')
            one = collection.find_one({'pay_id': bid})
            data['r_data'] = one
            template = get_template('admin/book_receipt.html')
            context = {
                "invoice_id": bid,
                "customer_name": uname,
                "address": uadd,
            ...
            html = template.render(context)
            pdf = render_to_pdf('admin/book_receipt.html', context)
            if pdf:
                response = HttpResponse(pdf, content_type='application/pdf')
                filename = "Invoice_%s.pdf" % ("12341231")
                content = "inline; filename='%s'" % (filename)
